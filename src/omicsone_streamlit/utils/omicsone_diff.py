...
def compare_two_files(file1, file2, method = 'Wilcoxon(Unpaired)', fdr_cutoff=0.01, log2fc_cutoff=1,
                       max_miss_ratio_global=0.5, max_miss_ratio_group=0.5, min_sample_size=2):
    df1 = pd.read_csv(file1,sep='\t',index_col=0)
    df2 = pd.read_csv(file2,sep='\t',index_col=0)
    df1.columns = [i + '.1' for i in df1.columns.values]
    df2.columns = [i + '.2' for i in df2.columns.values]
    a = df1.columns.values
    b = df2.columns.values
    df = pd.concat([df1,df2],axis=1)
    return compare_two_groups(df, a, b, method, fdr_cutoff, log2fc_cutoff,
                       max_miss_ratio_global, max_miss_ratio_group, min_sample_size)

def compare_two_groups(df, a, b, method = 'Wilcoxon(Unpaired)', fdr_cutoff=0.01, log2fc_cutoff=1,
                       max_miss_ratio_global=0.5, max_miss_ratio_group=0.5, min_sample_size=2):
    """
    Parameters
    ----------
